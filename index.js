...
module.exports = function Sitemap(siteURL, siteDistPath, dryRun, tld) {
  ...
  this.add = function (url, changeFreq, priority) {
    ...
    this.flush = function (callback) {
      ...
      fs.writeFileSync(siteDistPath + '/sitemap-index-' + tld + '.xml', xml);
      ...
      async.each(this.searchEngines, function (target, callback) {
        ...
        req.open("GET", "http://" + target.URL + "/ping?sitemap=" + siteURL + "/sitemap-index-" + tld + ".xml", true);
        req.send();
      }, function () {
        if (callback) {
          callback();
    ...
    this._flush = function (callback) {
      ...
      filename += this.sitemapCount + '-' + tld + '.xml';
